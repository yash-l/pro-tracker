...
# Default config (Will be overridden by Environment Variables if present)
DEFAULT_CONFIG = {
    "api_id": int(os.environ.get("API_ID", 9497762)),
    "api_hash": os.environ.get("API_HASH", "272c77bf080e4a82846b8ff3dc3df0f4"),
    "phone": os.environ.get("PHONE", "+918849404331"),
    "bot_api_key": os.environ.get("BOT_API_KEY", "7949002012:AAG9ifTdXAMIAk_vPFfOnM1wxHOela9o87w"),
    "chat_id": int(os.environ.get("CHAT_ID", 1184218529)),
    "timezone": os.environ.get("TIMEZONE", "Asia/Kolkata"),
    "admin_username": os.environ.get("ADMIN_USERNAME", "admin"),
    "admin_password": os.environ.get("ADMIN_PASSWORD", "password"),
    "recovery_key": os.environ.get("RECOVERY_KEY", secrets.token_hex(8)),
    "secret_key": os.environ.get("SECRET_KEY", secrets.token_hex(16)),
    "proxy_enabled": False,
    "proxy_ip": "127.0.0.1",
    "proxy_port": 8080,
    "is_setup_done": True # Defaults to True so it relies on Env Vars or existing config
}
...
# --- CONFIG LOADER ---
def load_config():
    # Priority: 1. config.json (if exists), 2. Environment Variables (via DEFAULT_CONFIG)
...
# Fix cookies for Android/Cross-site
...
async def get_hourly_data(user_id=None):
    ...
    try:
        async with aiosqlite.connect(DB_FILE) as db:
            async with db.execute(query, args) as cursor: rows = await cursor.fetchall()
        for row in rows:
...
async def log_event(user_id, status, time_str):
    try:
        async with aiosqlite.connect(DB_FILE) as db:
            ...
            await db.commit()
    except Exception as e: logger.error(f"Log Event Error: {e}")
...
async def tracker_loop():
    # SAFETY CHECK: Ensure session is valid before looping
    logger.info("Checking Telegram authorization...")
    try:
        await client.connect()
        if not await client.is_user_authorized():
            logger.critical("❌ SESSION INVALID! You must run this locally to generate session_pro.session then upload it.")
            return # Stop the loop, don't crash
    except Exception as e:
    ...
    while True:
        try:
            ...
            for (uid, name) in targets:
                try:
                    # Use cache if available to prevent FloodWait
                    if uid in entity_cache:
                        user = await client.get_entity(entity_cache[uid])
                    else:
                        user = await client.get_entity(uid)
                        entity_cache[uid] = user # Cache it
                    ...
                    logger.error(f"Error checking {uid}: {e}")
                
                await asyncio.sleep(1.5) # Slight delay between users
                
            await asyncio.sleep(5) # Delay between full cycles
...
<div class="auth-container"><div class="auth-card">
    <div class="title">噫 Initial Setup</div>
...
<div class="auth-container"><div class="auth-card">
    <div class="title">売 Recovery</div>
    <p style="text-align:center; color:#94a3b8; font-size:0.9rem;">Enter the Recovery Key shown in your Termux console/Logs.</p>
...
<div class="container">
    ...
    <div class="card">
        ...
        <div class="alert" style="background:rgba(59, 130, 246, 0.2); color:#93c5fd; border-color:rgba(59, 130, 246, 0.5); text-align:left; font-size:0.85rem;">
            <b>Note:</b> For permanent changes on Render, use "Environment Variables" in your Render Dashboard. Changes here will reset if the app restarts.
        </div>
        <form action="/update_profile" method="post">
            <h4 style="color:#94a3b8; margin-bottom:10px;">Login Credentials</h4>
            ...
            <h4 style="color:#94a3b8; margin-bottom:10px;">Telegram API Settings</h4>
            
            <label style="color:#94a3b8; font-size:0.9rem;">API ID</label>
            <input type="text" name="api_id" value="{{ cfg.api_id }}" class="input">
            
            <label style="color:#94a3b8; font-size:0.9rem;">API Hash</label>
            <input type="text" name="api_hash" value="{{ cfg.api_hash }}" class="input">
            
            <label style="color:#94a3b8; font-size:0.9rem;">Phone</label>
            <input type="text" name="phone" value="{{ cfg.phone }}" class="input">
            
            <label style="color:#94a3b8; font-size:0.9rem;">Bot API Key</label>
            <input type="text" name="bot_api_key" value="{{ cfg.bot_api_key }}" class="input">
            
            <label style="color:#94a3b8; font-size:0.9rem;">Chat ID (For Logs)</label>
            <input type="text" name="chat_id" value="{{ cfg.chat_id }}" class="input">
            
            <label style="color:#94a3b8; font-size:0.9rem;">Timezone</label>
            <input type="text" name="timezone" value="{{ cfg.timezone }}" class="input">
...
async def update_profile():
    ...
    c['api_hash'] = form['api_hash']
    c['phone'] = form['phone']
    c['bot_api_key'] = form['bot_api_key']
    c['chat_id'] = int(form['chat_id'])
    c['timezone'] = form['timezone']
    
    # Reset Session if phone changed
    if c['phone'] != cfg['phone'] and os.path.exists('session_pro.session'): 
...
@app.before_serving
async def startup():
    print(f"\n{'='*40}\n泊 RECOVERY KEY: {cfg['recovery_key']}\n{'='*40}\n")
    await init_db()
