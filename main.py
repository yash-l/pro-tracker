...
POLL_INTERVAL = 15  # Seconds between checks (Saves CPU)

os.makedirs(PIC_FOLDER, exist_ok=True)
# Reduce logging to errors only to save disk logs
        ...
        try:
            ...
            for uid, name in targets:
                try:
                    ...
                    # Heartbeat (Updates timestamp without flooding logs)
                    async with aiosqlite.connect(DB_FILE) as db:
                        await db.execute('UPDATE targets SET last_seen=? WHERE user_id=?', (now, uid))
                        await db.commit()
                except: pass
                # Tiny pause between users to prevent CPU spikes
                await asyncio.sleep(0.5) 
            
            # LONG SLEEP to save Free Tier CPU
...
async def index():
    ...
    return render_template_string(P_DASH.replace("{{ROWS}}", html if html else "<div class='text-center text-slate-500 text-sm'>No targets</div>"))
